...
ENGINE_DIR = "trt_merge_lora_model"
...
def main():
    ...
    # Tải các phụ kiện, không quan trọng dtype ở đây
    pipe = StableDiffusionPipeline.from_pretrained(model_id)
    scheduler = pipe.scheduler
    tokenizer = pipe.tokenizer

    # --- TẢI TOÀN BỘ ENGINE FP32 ---
    print("Loading TensorRT engines (FP32 pipeline)...")
    ...
    vae_decoder_engine = load_engine(os.path.join(ENGINE_DIR, "vae_decoder_fp32.plan"))
    ...
    # Text Encoder FP32 sẽ trả về hidden_states là FP32
    hidden_states, _ = infer(text_encoder_engine, [input_ids])

    # --- Diffusion loop (Toàn bộ là FP32) ---
    print("Running diffusion with UNet TRT...")
    # Tạo latents ban đầu là FP32
    ...
    for t in timesteps:
        ...
        # Tất cả đầu vào cho UNet đều là FP32 (hoặc int32)
        noise_pred = infer(unet_engine, [latents, timestep, hidden_states])[0]
        # Scheduler cũng sẽ làm việc với FP32
    ...
    # --- Decode image (Toàn bộ là FP32) ---
    print("Decoding with VAE decoder TRT...")
    # Đầu vào latents cho VAE là FP32, khớp với engine VAE FP32
    images = infer(vae_decoder_engine, [latents])[0]
